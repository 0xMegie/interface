...
the S03 app. It indexes SO4 Soroban contract activity from deployment manifests
generated by the contracts repo.

## Sync Deployed Contracts

After deploying or bootstrapping contracts, sync the generated manifest into the
indexer:
...
The command reads `.deployed/<network>.env`, `.deployed/tokens-<network>.env`,
`.deployed/frontend-<network>.env`, and
`.stellar/contract-ids/<network>.json` from the contracts repo, then writes
`config/contracts.<network>.json`.

By default the script checks the local contracts repo layout used by this
workspace and nearby `../contracts` style layouts. Set
`CONTRACTS_REPO_PATH=/path/to/contracts` or pass
`--contracts-repo /path/to/contracts` when your checkout is somewhere else.
...
Missing core contracts, test tokens, endpoints, or malformed contract IDs fail
fast. Missing `MARKET_TOKEN_*` triplets are only allowed by the local
pre-bootstrap command and are reported before the config is written.
